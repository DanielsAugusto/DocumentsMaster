import { useState, useEffect } from 'react';
import { createPortal } from 'react-dom';
import { Button } from '@/components/ui/button';
import { Select } from '@/components/ui/select';
import { X, FolderOutput } from 'lucide-react';
import { useMoveFolder, useAllFolders } from '../hooks/useFolders';
import { useQueryClient } from '@tanstack/react-query';
import { moveDocument } from '../api/folders';

type SelectedItem = { id: string, type: 'folder' | 'document', name: string };

interface MoveMultipleModalProps {
    isOpen: boolean;
    onClose: () => void;
    items: SelectedItem[];
    currentFolderId: string | null;
}

export function MoveMultipleModal({ isOpen, onClose, items, currentFolderId }: MoveMultipleModalProps) {
    const { data: allFolders = [], isLoading } = useAllFolders();
    const moveFolder = useMoveFolder();
    const queryClient = useQueryClient();
    const [selectedFolderId, setSelectedFolderId] = useState<string>('root');
    const [error, setError] = useState<string | null>(null);
    const [mounted, setMounted] = useState(false);
    const [isMoving, setIsMoving] = useState(false);

    useEffect(() => {
        setMounted(true);
    }, []);

    useEffect(() => {
        if (isOpen) {
            setSelectedFolderId(currentFolderId || 'root');
            setError(null);
        }
    }, [isOpen, currentFolderId]);

    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.key === 'Escape' && isOpen && !isMoving) {
                onClose();
            }
        };

        globalThis.addEventListener('keydown', handleKeyDown);
        return () => globalThis.removeEventListener('keydown', handleKeyDown);
    }, [isOpen, onClose, isMoving]);

    if (!isOpen || !mounted || items.length === 0) return null;

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setError(null);
        setIsMoving(true);

        const targetId = selectedFolderId === 'root' ? null : selectedFolderId;

        if (targetId === currentFolderId) {
            setIsMoving(false);
            onClose();
            return;
        }

        try {
            const folderIds = items.filter(i => i.type === 'folder').map(i => i.id);
            const docIds = items.filter(i => i.type === 'document').map(i => i.id);

            const promises: Promise<any>[] = [];

            for (const fId of folderIds) {
                promises.push(moveFolder.mutateAsync({ folderId: fId, targetParentId: targetId }));
            }

            for (const dId of docIds) {
                promises.push(moveDocument(dId, targetId));
            }

            await Promise.all(promises);

            queryClient.invalidateQueries({ queryKey: ['folders'] });
            queryClient.invalidateQueries({ queryKey: ['documents'] });
            onClose();
        } catch (err: any) {
            setError(err.message || 'Erro ao mover itens.');
        } finally {
            setIsMoving(false);
        }
    };

    // Filter out folders that are within the current selected folders, or identical to them
    const selectedFolderIds = new Set(items.filter(i => i.type === 'folder').map(i => i.id));
    const validFolders = allFolders.filter(f => !selectedFolderIds.has(f.id));

    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/50 backdrop-blur-sm transition-all animate-in fade-in duration-200">
            <button type="button" className="fixed inset-0 bg-transparent border-none cursor-default" tabIndex={-1} onClick={() => !isMoving && onClose()} aria-label="Fechar modal" />
            <div className="relative bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 border border-gray-100 dark:border-gray-800">
                <form onSubmit={handleSubmit} className="p-6">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-2">
                            <FolderOutput className="h-5 w-5 text-gray-500" />
                            <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                                {items.length === 1 ? 'Mover 1 item' : `Mover ${items.length} itens`}
                            </h3>
                        </div>
                        <button
                            type="button"
                            onClick={() => !isMoving && onClose()}
                            className="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300 transition-colors"
                            disabled={isMoving}
                        >
                            <X className="h-5 w-5" />
                        </button>
                    </div>

                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
                        Você selecionou {items.length === 1 ? '1 item' : `${items.length} itens`}. Para onde deseja enviá-{items.length === 1 ? 'lo' : 'los'}?
                    </p>

                    <div className="mb-6 space-y-4">
                        <div>
                            <label htmlFor="folderSelect" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                Pasta de Destino
                            </label>
                            {isLoading ? (
                                <div className="h-10 bg-gray-100 dark:bg-gray-800 animate-pulse rounded"></div>
                            ) : (
                                <Select
                                    id="folderSelect"
                                    value={selectedFolderId}
                                    onChange={(e) => setSelectedFolderId(e.target.value)}
                                    className="w-full text-gray-900 dark:text-white bg-white dark:bg-gray-950"
                                    disabled={isMoving}
                                >
                                    <option value="root">Raiz (Tela Inicial)</option>
                                    {validFolders.map(folder => (
                                        <option key={folder.id} value={folder.id} disabled={folder.id === currentFolderId}>
                                            {folder.name} {folder.id === currentFolderId ? '(Atual)' : ''}
                                        </option>
                                    ))}
                                </Select>
                            )}
                            {error && <p className="mt-2 text-sm text-red-500">{error}</p>}
                        </div>
                    </div>

                    <div className="flex flex-col sm:flex-row gap-3 justify-end">
                        <Button
                            type="button"
                            variant="outline"
                            onClick={onClose}
                            className="w-full sm:w-auto hover:bg-gray-100 dark:hover:bg-slate-800"
                            disabled={isMoving}
                        >
                            Cancelar
                        </Button>
                        <Button
                            type="submit"
                            className="w-full sm:w-auto"
                            disabled={isMoving || isLoading}
                        >
                            {isMoving ? 'Movendo...' : 'Mover'}
                        </Button>
                    </div>
                </form>
            </div>
        </div>,
        document.body
    );
}
